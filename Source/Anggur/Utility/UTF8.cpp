...
namespace Anggur::UTF8
{
	...
	uint Pack(const std::string& text, size_t index) 
	{
		...
		{
			// Three-bytes
			codepoint = ((text[index] & 0b00001111) << 12) | ((text[index+1] & 0b00111111) << 6) | (text[index+2] & 0b00111111);
		}
		else if ((text[index] & 0b11111000) == 0b11110000) 
		{
			// Four-bytes 
			codepoint = ((text[index] & 0b00000111) << 18) | ((text[index+1] & 0b00111111) << 12) | ((text[index+2] & 0b00111111) << 6) | (text[index+3] & 0b00111111);
		} 

		return codepoint;
	}

	uint PackAndMoveIndex(const std::string& text, size_t& index) 
	{
		// std::string text = "Hello, 🌎!";
		uint codepoint = 0;
